...
public class VerticalStepperItemView extends FrameLayout {
	...
	public VerticalStepperItemView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
		...
		DP = getResources().getDimensionPixelSize(R.dimen._1sdp);
	...
	public synchronized void setState(@State int state) {
		...
		if (state != STATE_NORMAL && mState == STATE_NORMAL) {
			...
			mPointFrame.setBackgroundResource(R.drawable.stepper_circle_inactive);
			...
			mPointFrame.setBackgroundResource(state == STATE_NORMAL ? R.drawable.stepper_circle_inactive : R.drawable.stepper_circle);

			mPointBackground.setBackgroundColor(state == STATE_NORMAL ? mNormalColor : mActivatedColor);
		}

		if(state == STATE_SELECTED){
			mPointFrame.setBackgroundResource(R.drawable.stepper_circle);
			mLineView.setBackgroundColor(getResources().getColor(R.color.livo_green));
		}
		...
		if (mErrorText != null) {
			...
			if (mSummaryColorAnimator != null) mSummaryColorAnimator.cancel();
			mSummaryColorAnimator = ViewUtils.createArgbAnimator(
					mSummaryText, "textColor", mSummaryText.getCurrentTextColor(), getResources().getColor(
