...
class PickupFragment : Fragment(), OnMapReadyCallback {
    ...
    private val AUTOCOMPLETE_REQUEST_CODE = 61
    ...
    override fun onCreateView(
        ...
        var root = inflater.inflate(R.layout.fragment_pickup, container, false)
    ...
    fun initViews() {
        ...
        for (date in dates!!) {
            datedayList?.add(
                DateDayModel(
                    date.date.toString(),
                    date.day.toString(),
                    SimpleDateFormat("yyyy").format(date) as String + "-" + SimpleDateFormat("MM").format(
                        date
                    ) as String + "-" + SimpleDateFormat("dd").format(date) as String
                )
            )
        }

        rvDateList.setHasFixedSize(true)
        rvDateList.layoutManager = LinearLayoutManager(
            currActivity,
            LinearLayoutManager.HORIZONTAL, false
        )
        var adapter = PickUpAdapter(currActivity!!, datedayList, 0)
        rvDateList.adapter = adapter


        // Initialize bundle
        (currActivity as NewListingActivity).addressBundle = Bundle()
        //intitalization of FusedLocationProviderClient
        fusedLocationProviderClient =
            LocationServices.getFusedLocationProviderClient(currActivity!!)
        ...
        if (i != null) {
            val extras = i.extras
            if (extras != null) {
                (currActivity as NewListingActivity).mAddress = extras.getString(MapUtility.ADDRESS)
                //temp -> get lat , log from db
                (currActivity as NewListingActivity).mLatitude =
                    currActivity!!.intent.getDoubleExtra(MapUtility.LATITUDE, 0.0)
                (currActivity as NewListingActivity).mLongitude =
        ...
        etAddressNote.addTextChangedListener(object : TextWatcher {
            ...
            override fun onTextChanged(p0: CharSequence?, p1: Int, p2: Int, p3: Int) {
                (currActivity as NewListingActivity).addressNote = p0!!.toString()
                tvCharacterCount.text =
                    "" + (currActivity as NewListingActivity).productTitle.length + activity!!.resources.getString(
                        R.string.count_characters120
                    )
            }
    ...
    fun getLocationFromAddress(context: Context?, strAddress: String?): LatLng? {
        ...
        try {
            ...
            if (address.isNotEmpty()) {
                ...
                (currActivity as NewListingActivity).mLatitude = location.latitude
                (currActivity as NewListingActivity).mLongitude = location.longitude
                addMarker()
            }
    ...
    private fun addMarker() {
        ...
        if (mMap != null) {
            ...
            try {
                ...
                markerOptions = MarkerOptions().position(coordinate)
                    .title((currActivity as NewListingActivity).mAddress).icon(
        ...
        try {
            (currActivity as NewListingActivity).userAddressline2 =
                (currActivity as NewListingActivity).userAddressline2!!.substring(
                    0,
                    (currActivity as NewListingActivity).userAddressline2!!.indexOf(
                        (currActivity as NewListingActivity).userCity!!
                    )
                )
        ...
        try {
            addressline2?.setText((currActivity as NewListingActivity).userAddressline2)
            citydetail!!.text =
                (currActivity as NewListingActivity).userCity + SPACE +
                        (currActivity as NewListingActivity).userPostalCode +
                        SPACE + (currActivity as NewListingActivity).userState +
                        SPACE + (currActivity as NewListingActivity).userCountry
        } catch (ex: Exception) {
            ex.printStackTrace()
        }
    }
    ...
    override fun onMapReady(googleMap: GoogleMap) {
        ...
        mMap!!.setInfoWindowAdapter(object : GoogleMap.InfoWindowAdapter {
            ...
            override fun getInfoContents(arg0: Marker): View {
                ...
                (currActivity as NewListingActivity).mLatitude = latLng.latitude
                (currActivity as NewListingActivity).mLongitude = latLng.longitude
                val tvLat = v.findViewById<TextView>(R.id.address)
                tvLat.text = (currActivity as NewListingActivity).mAddress
                return v
            }
        })
            ...
            lastLocation.addOnSuccessListener(
                ...
                if (location != null) {
                    ...
                    if (isDirectionClicked) {
                        ...
                        val mLatitude = (currActivity as NewListingActivity).mLatitude
                        val mLongitude = (currActivity as NewListingActivity).mLongitude
...
//                ).show()
    ...
    private fun startParsingAddressToShow() {
        ...
        if ((currActivity as NewListingActivity).mAddress == null || (currActivity as NewListingActivity).mAddress!!.isEmpty()) {
        ...
        //format will be lat,lng i.e 19.23234,72.65465
            if (latLongPattern.matcher((currActivity as NewListingActivity).mAddress).matches()) {
                val p =
                    Pattern.compile("(-?\\d+(\\.\\d+)?)") // the pattern to search for
                val m = p.matcher((currActivity as NewListingActivity).mAddress)

                // if we find a match, get the group
                var i = 0
                while (m.find()) {
                    // we're only looking for 2s group, so get it
                    if (i == 0) (currActivity as NewListingActivity).mLatitude =
                        m.group().toDouble()
                    if (i == 1) (currActivity as NewListingActivity).mLongitude =
                        m.group().toDouble()
                    i++
                }
                //show on map
                ...
                if ((currActivity as NewListingActivity).mLatitude == 0.0 &&
                    (currActivity as NewListingActivity).mLongitude == 0.0
                ) {
    ...
    private inner class GetAddressFromLatLng :
        ...
        protected override fun doInBackground(vararg p0: Double?): Bundle? {
            return try {
                ...
                if (addresses != null && addresses.size > 0) {
                    ...
                    if (address != null) (currActivity as NewListingActivity).addressBundle!!.putString(
                        "addressline2",
                        address
                    )
                    sb.append(address).append(" ")
                    val city = addresses[0].locality
                    if (city != null) (currActivity as NewListingActivity).addressBundle!!.putString(
                        "city",
                        city
                    )
                    ...
                    (currActivity as NewListingActivity).addressBundle
                } else {
                    null
                }
            } catch (e: IOException) {
                e.printStackTrace()
                (currActivity as NewListingActivity).addressBundle!!.putBoolean("error", true)
                (currActivity as NewListingActivity).addressBundle
        ...
        override fun onPostExecute(userAddress: Bundle?) {
            ...
            (currActivity as NewListingActivity).mAddress =
                userAddress!!.getString("fulladdress").toString()
            (currActivity as NewListingActivity).userCity = userAddress.getString("city")
            (currActivity as NewListingActivity).userState = userAddress.getString("state")
            (currActivity as NewListingActivity).userPostalCode =
                userAddress.getString("postalcode")
            (currActivity as NewListingActivity).userCountry = userAddress.getString("country")
            (currActivity as NewListingActivity).userAddressline2 =
                userAddress.getString("addressline2")
            MapUtility.hideProgress()
            addMarker()
        }
    }
        ...
        protected override fun doInBackground(vararg p0: String?): LatLng? {
            ...
            try {
                ...
                addresses = geocoder.getFromLocationName(
                    (currActivity as NewListingActivity).mAddress?.get(0).toString(), 1
                )
                if (addresses != null && addresses.size > 0) {
    ...
    private val settingsLocation: Unit
        private get() {
            ...
            val result =
                LocationServices.getSettingsClient(currActivity!!)
